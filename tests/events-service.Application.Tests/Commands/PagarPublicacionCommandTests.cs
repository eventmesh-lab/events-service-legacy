...
namespace events_service.Application.Tests.Commands
{
    ...
    public class PagarPublicacionCommandTests
    {
        ...
        private static Mock<IValidator<PagarPublicacionCommand>> CreateValidatorMock(bool isValid = true)
        {
            var mock = new Mock<IValidator<PagarPublicacionCommand>>();
            var result = isValid 
                ? new FluentValidation.Results.ValidationResult() 
                : new FluentValidation.Results.ValidationResult(new[] { new FluentValidation.Results.ValidationFailure("Test", "Error") });
            
            mock.Setup(v => v.ValidateAsync(It.IsAny<PagarPublicacionCommand>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(result);
            return mock;
        }
